...
class Main:
    ...
    def edge_detection(self) -> None:
        ...
        for image_file in tqdm(image_files, desc="Processing images"):
            ...
            for method in edge_methods:
                ...
                results.append({"title": f"{file_name}-{method}", "image": edge_image})
    ...
    def jpeg_compression(self) -> None:
        ...
        for image_file in tqdm(image_files, desc="Compressing images"):
            ...
            plotter.save_plot(
                data=[
                    {
                        "title": f"{file_name}-original-image",
                        "image": img.convert_to_rgb(image),
                    },
                    {
                        "title": f"{file_name}-compressed-image",
                        "image": img.convert_to_rgb(compressed_image),
                    },
                    {
                        "title": f"{file_name}-decompressed-image",
                        "image": img.convert_to_rgb(decompressed_image),
                    },
                ],
            )
    ...
    def image_inpainting_by_blind_mask(self, reverse: bool = False) -> None:
        """
...
if __name__ == "__main__":
    ...
    # # 29. Process image inpainting by blind mask. (Reverse)
    # main.image_inpainting_by_blind_mask(reverse=True)
